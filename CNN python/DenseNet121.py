...
def load_bfm(dir, count_file, spot_locations, number_of_genes_to_predict):
    ...
    if barcodes == sorted_barcodes: #todo make sure bfm and spot_locations really do match!
...
def main():
    ...
    use_max_samples = True  # This will overwrite the number_of_samples_to_use if True
    ...
    if len(file) >= 1:
        print("Importing data from " + str(len(file)) + " data sets")
    ...
    for i in file:
        ...
        if use_max_samples:
            number_of_samples_to_use = len(bfm)
        train_indexes, test_indexes = generate_train_test_indexes(number_of_samples_to_use)
        ...
        training_cropped_img_list_i, testing_cropped_img_list_i = get_cropped_images(image_path,
                                                                                    ...
                                                                                    train_indexes,
                                                                                    test_indexes)
        print(training_cropped_img_list_i.shape)
    ...
    print(training_cropped_img_list.shape)
    print(training_bfm.shape)
